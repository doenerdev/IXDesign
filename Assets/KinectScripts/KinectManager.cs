...
public class KinectManager : MonoBehaviour
{
    ...
    public List<AvatarControllerLocal> avatarControllers = new List<AvatarControllerLocal>();
    ...
    public Vector3 GetUserPosition(Int64 userId)
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            ...
            {
                //NH needs to be removed later
                var position = new Vector3(-bodyFrame.bodyData[index].position.x, bodyFrame.bodyData[index].position.y, bodyFrame.bodyData[index].position.z);
                //return position;
    ...
    public Quaternion GetJointOrientation(Int64 userId, int joint, bool flip)
    {
        if (dictUserIdToIndex.ContainsKey(userId))
        {
            ...
            if (index >= 0 && index < sensorData.bodyCount &&
               bodyFrame.bodyData[index].bIsTracked != 0 &&
               bodyFrame.bodyData[index].joint[joint].trackingState == KinectInterop.TrackingState.Tracked) //added query for the joints tracking state
            {
                if (flip)
    ...
    void StartKinect()
    {
        ...
        if (avatarControllers.Count == 0)
        {
            ...
            foreach (MonoBehaviour monoScript in monoScripts)
            {
                ...
                if ((monoScript is AvatarControllerLocal) && monoScript.enabled)
                {
                    AvatarControllerLocal avatar = (AvatarControllerLocal)monoScript;
    ...
    void Update()
    {
        if (kinectInitialized)
        {
            ...
            {
                foreach (AvatarControllerLocal controller in avatarControllers)
                {
                    ...
                    if (userId != 0 && dictUserIdToIndex.ContainsKey(userId))
                    {
                        ...
                        controller.UpdateTrackingState(true);
                    }
                     //AH
                    else
                    {
                        //TODO Trigger Animation starten
                        Debug.Log(controller.gameObject.transform.position);
                        //controller.ResetToInitialPosition(false);
                    }
                }
            }
		 ...
		 //AH
        calibrationText.text = PrintUserInfo();
    }
    ...
    void LateUpdate()
    {
        ...
        if (lateUpdateAvatars)
        {
            foreach (AvatarControllerLocal controller in avatarControllers)
            {
                ...
                {
                    controller.UpdateTrackingState(false);
                }
            }
        }
    }
    ...
    private void ProcessBodyFrameData()
    {
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
        {
            ...
            // && (maxTrackedUsers < 0 || trackedUsers < maxTrackedUsers))
            {
				...
				//AH
                //If body is in tracking area calc the respective Engagement level, otherwise set to unknown
                if (Mathf.Abs(bodyData.position.z) >= minUserDistance &&
                    (maxUserDistance < 0.51f || Mathf.Abs(bodyData.position.z) <= maxUserDistance) &&
                    (maxLeftRightDistance < 0.01f || Mathf.Abs(bodyData.position.x) <= maxLeftRightDistance))
                {
                    FacetrackingManager.CalcEngagementLevel(userId);
                }
                 //AH
				else
				{
					if (dictUserIdToTime.ContainsKey(userId))
					{
						// Avoid sporadical tracking failures
						if (Time.time - dictUserIdToTime[userId] > waitTimeBeforeRemove)
						{
							FacetrackingManager.SetUserEngagement(userId, FacetrackingManager.EngagementLevel.Unknown, Time.time);
						}
					}
				}
			}
        ...
        if (!alUserIds.Contains(userId))
        {
            if (CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
            {
                ...
                if (liPrimaryUserId == 0 && aUserIndexIds.Length > 0)
                {
                    ...
                    liPrimaryUserId = FacetrackingManager.GetClosestEngagedUser(ref aUserIndexIds);
                ...
                {
                    AvatarControllerLocal avatar = avatarControllers[i];
    ...
    protected virtual void RemoveUser(Int64 userId)
    {
        ...
        for (int i = 0; i < avatarControllers.Count; i++)
        {
            ...
            {
                //avatar.ResetToInitialPosition(false);
                avatar.playerId = 0;
            }
        }
        ...
        alUserIds.Remove(userId);
		 ...
		 //AH
        FacetrackingManager.RemoveUserEngagement(userId);
        PropsManager.Instance.FadeOutProp(userId);
    ...
    public float BodyTurningThreshold = 40;

    private FacetrackingManager FacetrackingManager;


    private void DetermineNewPrimaryUser()
    {
        if (FacetrackingManager.IsEngaged(liPrimaryUserId)) return;
        ...
        // calibrates the respective avatar controllers
        foreach (AvatarControllerLocal ctrl in avatarControllers)
        {
            if (ctrl.playerIndex == 0)
            {
                ctrl.playerId = liPrimaryUserId;
            }
        }
    }
    ...
    {
        if (PropsManager.Instance != null)
        {
            foreach (var userId in alUserIds)
            {
                if (userId == liPrimaryUserId && PropsManager.Instance.IsProp(userId) &&
                    !PropsManager.Instance.IsPropFadingOut(userId))
                {
                    PropsManager.Instance.FadeOutProp(userId);
                }
                else if (userId != liPrimaryUserId && !PropsManager.Instance.IsProp(userId))
                {
                    PropsManager.Instance.AddProp(userId);
                }
            }
        }
        else
        {
            Debug.Log("PropsManager Instance is null");
        }
    }
    ...
    public string PrintUserInfo()
    {
        ...
        if (liPrimaryUserId != 0)
        {
            text += string.Format("Index: {0}, UserID: {1}, Body: {2}, Eng: {3}, User Count: {4} \n",
                GetUserIndexById(liPrimaryUserId), liPrimaryUserId.ToString().Substring(15),
                GetBodyIndexByUserId(liPrimaryUserId), FacetrackingManager.GetEngagementLevel(liPrimaryUserId), alUserIds.Count);
        }
        else
        {
            text += "No Primary User \n";
        }
        ...
        if (alUserIds.Count != 0)
        {
            for (int i = 0; i < aUserIndexIds.Length; i++)
            {
                if (aUserIndexIds[i] != 0)
                {
                    text += string.Format("Index: {0}, UserID: {1}, Body: {2}, Eng: {3} \n", i,
                        aUserIndexIds[i].ToString().Substring(15), GetBodyIndexByUserId(aUserIndexIds[i]),
                        FacetrackingManager.GetEngagementLevel(aUserIndexIds[i]));
                }
            }
        }
        else
    ...
    //float minBodyRot = -30;
    //float maxBodyRot = 30;
    //float minAvatarRot = -90;
    //float maxAvatarRot = 90;

    //public float BodyTurningTargetAngle = 45;
    //public float BodyTurningDuration = 1;
    //private float BodyTurningStart;
    //private bool StartTurning;
    //private bool BodyTurned;

    public Quaternion GetExternalRootRotation()
    {
        float bodyTurnAngle = bodyFrame.bodyData[dictUserIdToIndex[liPrimaryUserId]].bodyTurnAngle;
        float bodyTurnAngleNormalized = bodyTurnAngle > 180 ? bodyTurnAngle - 360 : bodyTurnAngle;

        //Limit Avatar rotation 
        if (Mathf.Abs(bodyTurnAngleNormalized) > BodyTurningThreshold)
        {
            return Quaternion.Euler(new Vector3(0, Mathf.Sign(bodyTurnAngleNormalized) * (Mathf.Abs(bodyTurnAngleNormalized) - BodyTurningThreshold), 0));
        }

        return Quaternion.Euler(new Vector3(0, 0, 0));

        //Debug.Log(bodyTurnAngleNormalized);

        //if (Mathf.Abs(bodyTurnAngleNormalized) > BodyTurningThreshold)
        //{
        //    if (!StartTurning && !BodyTurned)
        //    {
        //        StartTurning = true;
        //        BodyTurningStart = Time.time;
        //    }
        //    else
        //    {
        //        y = Mathf.Sign(bodyTurnAngleNormalized) * (BodyTurningTargetAngle - BodyTurningThreshold);
        //    }
        //}
        //else
        //{
        ...
        //if (StartTurning && !BodyTurned)
        //{
        //    float t = (Time.time - BodyTurningStart) / BodyTurningDuration;
        ...
        //    else
        //    {
        //        y = Mathf.SmoothStep(bodyTurnAngleNormalized, Mathf.Sign(bodyTurnAngleNormalized) * (BodyTurningTargetAngle - BodyTurningThreshold), t);
        ...
        //float bodyTurnAngleClamped = Mathf.Clamp(bodyTurnAngleNormalized, minBodyRot, maxBodyRot);
        //float bodyTurnAngleMapped = Map(bodyTurnAngleClamped, minBodyRot, maxBodyRot, minAvatarRot - minBodyRot,
        //    maxAvatarRot - maxBodyRot);

        //Debug.Log(bodyTurnAngleClamped + " | " + bodyTurnAngleMapped);
